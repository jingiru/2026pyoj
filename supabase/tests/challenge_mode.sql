...
begin
  foreach role_name in array array['anon', 'authenticated'] loop
    foreach table_name in array array['challenges','challenge_participants','challenge_submissions','challenge_events'] loop
  ...
  insert into public.challenges(entry_code,title,duration_minutes,problem_snapshots) values(upper(substr(replace(gen_random_uuid()::text,'-',''),1,8)), '자동 검증용', 10, '[{"id":"test-p1","testCases":[{"input":"","output":"1"}]}]') returning id into c;
  ...
  if (select count(*) from public.challenge_events where challenge_id = c) <> 2 then raise exception 'Missing timing audit'; end if;
end;
$$;
